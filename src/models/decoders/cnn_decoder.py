...
class UpsampleBlock(nn.Module):
    ...
    After upsampling, fuses the projected original-image features at the
    same spatial resolution. The fusion mode is configurable so that image
    guidance can be ablated alongside the other fusion positions.
    """
    ...
    def __init__(
        ...
        # "concat" keeps the legacy 1×1 conv path so existing baselines remain
        # bit-exact; other modes route through MultiScaleFusionBlock.
        if img_fusion_mode == "concat":
            self.img_fuse = nn.Conv2d(out_channels + img_proj_channels, out_channels, 1, bias=True)
            self.img_fusion_block = None
        else:
            from src.models.modules.fusion import MultiScaleFusionBlock
            self.img_fuse = None
            self.img_fusion_block = MultiScaleFusionBlock(
                main_ch=out_channels,
                skip_channels=[img_proj_channels],
                mode=img_fusion_mode,
            )
    ...
    def forward(self, x: Tensor, img_feat: Tensor) -> Tensor:
        ...
        if self.img_fuse is not None:
            x = self.img_fuse(torch.cat([x, img_feat], dim=1))
        else:
            x = self.img_fusion_block(x, [img_feat], target_size=x.shape[-2:])
        return self.res(x)


def _factorize_token_grid(num_tokens: int) -> tuple[int, int]:
    """Infer a plausible 2D grid for flattened patch tokens."""
...
class CNNDecoder(BaseDecoder):
    ...
    def __init__(
        ...
        if not predict_residual:
            raise ValueError("CNNDecoder only supports residual output mode")
    ...
    def forward(self, x: Tensor, h: int, w: int, img: Tensor | None = None) -> Tensor:
        ...
        if token_count == h * w:
            ...
            src_h, src_w = _factorize_token_grid(token_count)
            x = x.transpose(1, 2).contiguous().reshape(B, -1, src_h, src_w)
