...
class ImageEnhancementPipeline(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # ── Main Encoder (frozen, no gradient) ──
        with torch.no_grad():
            if self.encoder_skip_layers:
                features, dino_skips = self.encoder(x, return_layer_indices=self.encoder_skip_layers)
                skip_dict = None
            elif hasattr(self.encoder, "skip_enabled") and self.encoder.skip_enabled:
                # New skip connection architecture (progressive multi-scale)
    ...
    def trainable_parameters(self) -> Iterator[nn.Parameter]:
        ...
        """
        if self.cnn_encoder is not None:
            # Only yield trainable parameters from cnn_encoder
            for param in self.cnn_encoder.parameters():
                if param.requires_grad:
                    yield param
        if self.cnn_proj is not None:
            yield from self.cnn_proj.parameters()
        if self.cnn_fusion_proj is not None:
            yield from self.cnn_fusion_proj.parameters()
        if self.cnn_fusion_block is not None:
            yield from self.cnn_fusion_block.parameters()
        if self.dino_skip_proj is not None:
            yield from self.dino_skip_proj.parameters()
        if self.main_encoder_proj is not None:
            yield from self.main_encoder_proj.parameters()
