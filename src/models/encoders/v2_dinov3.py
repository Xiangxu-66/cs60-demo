        ...
        # Multi-branch fusion
            ...
            if self.enable_dino:
                fusion_input = torch.cat([
                    ...
                    colour_feat if colour_feat is not None else torch.zeros_like(dino_feat),
                    local_feat  if local_feat  is not None else torch.zeros_like(dino_feat),
                ], dim=1)
                freq_gate, local_gate = self.fusion_gate(fusion_input).chunk(2, dim=1)

                enhanced_feat = dino_feat
                if colour_feat is not None:
