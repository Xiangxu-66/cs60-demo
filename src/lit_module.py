...
class ColorEnhanceLitModule(pl.LightningModule):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.save_test_comparison = bool(
            cfg.evaluation.get("save_test_comparison", True)
        )
        self.test_comparison_num_samples = int(
            cfg.evaluation.get(
                "test_comparison_num_samples",
                cfg.evaluation.get("num_visualization_samples", 8),
            )
        )
        self._saved_test_comparison = False
    ...
    def _artifact_paths(self) -> dict[str, str]:
        """Collect common artifact locations for this experiment."""
        exp = self.cfg.experiment_name
        config_dir = Path(self.cfg.logging.log_dir) / exp / "config"
        figures_dir = Path("outputs/figures") / exp
        eval_dir = Path("outputs/eval") / exp
        return {
            "config_yaml": str(config_dir / "config.yaml"),
            "config_json": str(config_dir / "config.json"),
            "figures_dir": str(figures_dir),
            "eval_dir": str(eval_dir),
            "test_comparison": str(eval_dir / "test_comparison.png"),
        }

    @staticmethod
    def _scalar_metrics(metrics: dict[str, Tensor]) -> dict[str, float]:
        """Convert scalar tensor metrics to plain floats."""
        out: dict[str, float] = {}
    ...
    def _run_summary(self) -> dict[str, object]:
        """Build a compact machine-readable summary of the current run."""
        checkpoint_cb = getattr(self.trainer, "checkpoint_callback", None)
        summary = {
            ...
            "model": {
                ...
                "encoder_target": str(self.cfg.encoder.get("_target_", "unknown")),
                "bottleneck_target": str(self.cfg.bottleneck.get("_target_", "unknown")),
                "decoder_target": str(self.cfg.decoder.get("_target_", "unknown")),
            },
            "training": OmegaConf.to_container(self.cfg.training, resolve=True),
            "data": {
                "data_dir": str(self.cfg.data.get("data_dir", "")),
                "expert": str(self.cfg.data.get("expert", "")),
                "batch_size": int(self.cfg.data.get("batch_size", 0)),
                "image_size": int(self.cfg.data.get("image_size", 0)),
                "crop_size": int(self.cfg.data.get("crop_size", 0)),
                "train_subset_size": self.cfg.data.get("train_subset_size"),
                "val_subset_size": self.cfg.data.get("val_subset_size"),
                "test_subset_size": self.cfg.data.get("test_subset_size"),
            },
            "loss": OmegaConf.to_container(self.cfg.loss, resolve=True),
            "artifacts": self._artifact_paths(),
            "logged_metrics": self._scalar_metrics(dict(self.trainer.logged_metrics)),
        }
        if checkpoint_cb is not None:
            ...
            summary["checkpoint"] = {
                ...
                "mode": checkpoint_cb.mode,
                "save_top_k": checkpoint_cb.save_top_k,
            }
        return summary

    def _write_summary_json(self, path: str | Path) -> None:
        """Persist a structured run summary JSON file."""
        path = Path(path)
    ...
    def test_step(
        ...
        if self.save_test_comparison and batch_idx == 0 and not self._saved_test_comparison:
            n = min(self.test_comparison_num_samples, pred.shape[0])
            eval_dir = Path("outputs/eval") / self.cfg.experiment_name
            save_comparison_grid(
                self._to_01(batch[0])[:n].cpu(),
                pred[:n].cpu(),
                target[:n].cpu(),
                save_path=eval_dir / "test_comparison.png",
            )
            self._saved_test_comparison = True

    def on_fit_end(self) -> None:
        """Save a compact summary after fit completes."""
        out_dir = Path(self.cfg.logging.results_dir)
        self._write_summary_json(out_dir / f"{self.cfg.experiment_name}_fit_summary.json")
    ...
    def configure_optimizers(self):
        """Build AdamW optimizer (bottleneck + decoder only) + LR scheduler.

        The encoder is frozen and never appears in the optimiser.
        """
