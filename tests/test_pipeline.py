...
class ResolutionAwareBottleneck(BaseBottleneck):
    def __init__(self, dim: int = 8) -> None:
        super().__init__()
        self.proj = nn.Linear(dim, dim)
        self._output_dim = dim
    ...
    def forward(self, x: Tensor, resolution: tuple[int, int] | None = None) -> Tensor:
...
def test_pipeline_passes_explicit_resolution_to_bottleneck() -> None:
    bottleneck = ResolutionAwareBottleneck()
    pipeline = ImageEnhancementPipeline(
        DummyEncoder(patch_size=14),
        bottleneck,
        DummyDecoder(),
    ...
    inputs = torch.rand(2, 3, 42, 112)  # 3 x 8 patch grid
    outputs = pipeline(inputs)

    assert tuple(outputs.shape) == tuple(inputs.shape)
    assert bottleneck.last_resolution == (3, 8)
