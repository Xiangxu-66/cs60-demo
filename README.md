# CS60 Portfolio Demo: RWKV-Based Image Color Aesthetic Enhancement

This repository is a portfolio/demo version of a university capstone codebase. It is intentionally separated from the original team repository and excludes private datasets, trained weights, experiment outputs, virtual environments, and any school-submission materials that should not be redistributed.

The original private upstream repository was not made public. This version is designed for employer review of the technical direction, code structure, and reproducible entry points.
...
My work focused on model and experiment engineering for image color enhancement:

- Implemented and compared RWKV-style bottlenecks against CNN and Transformer-style baselines.
- Maintained Hydra configuration groups for encoders, bottlenecks, decoders, losses, data recipes, and ablation runs.
- Built training, evaluation, visualization, reporting, and benchmark scripts.
- Added tests around datasets, model pipeline behavior, losses, metrics, and selected RWKV/LUT decoder components.
- Prepared reproducible environment files for CUDA and Apple Silicon/MPS development.
...
- Training/evaluation/reporting scripts
- Tests and environment setup files
- Architecture and experiment notes under `docs/`

Excluded:

- Private or licensed datasets
- Model checkpoints and trained weights
- Large generated outputs, result folders, and local environments
- Any confidential school, team, or assessment-only material
